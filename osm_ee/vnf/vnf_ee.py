...
def wait_for_ssh_to_be_ready(host: str, port: int, user: str, passwd: str, timeout: int, retry_interval: float) -> bool:
    global_logger.debug(
        f"Starting SSH connection to {host}:{port} as user <{user}> and passwd <{passwd}>. Timeout is {timeout}, retry interval is {retry_interval}")
    ...
    async def flexops(self, id_, params):
        ...
        try:
            ...
            if not wait_for_ssh_to_be_ready(
                    ...
                    5):
                self.logger.error(
                    "Cannot establish connection to ssh server: {}".format(self.config_params["ssh-hostname"]))
